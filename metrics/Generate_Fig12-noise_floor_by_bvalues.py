...
mask = get_data(f'/Simulations/Experiments/{exp}/Dataset/mean_f1samples_mask_new.nii.gz')
ground_truth = get_data(f'/Simulations/Experiments/{exp}/Dataset/noisyfree_data_full_b0_first.nii.gz')
bvals = np.genfromtxt(f'/Simulations/Experiments/{exp}/Dataset/data_b0b1b2k.bval', dtype=np.float32)
...
plt.tight_layout()
plt.savefig(f'{dPath}/figures/Fig12-{noise_type}_{iter}_noise_floor_by_bvalues_screenshots.png')
# Mostrar la gráfica
plt.show()
